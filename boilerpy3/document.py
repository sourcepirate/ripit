...
class TextBlock:
    ...
    def __init__(
        self,
        text: str,
        contained_text_elements=None,
        num_words: int = 0,
        num_words_in_anchor_text: int = 0,
        num_words_in_wrapped_lines: int = 0,
        num_wrapped_lines: int = 0,
        offset_blocks: int = 0,
    ) -> None:
    ...
    def _init_densities(self) -> None:
        ...
        self.text_density = self.num_words_in_wrapped_lines / float(
            self.num_wrapped_lines
        )
        self.link_density = (
            0
            if self.num_words == 0
            else self.num_words_in_anchor_text / float(self.num_words)
        )
    ...
    def merge_next(self, next_text_block: "TextBlock") -> None:
        ...
        self.offset_blocks_end = max(
            self.offset_blocks_end, next_text_block.offset_blocks_end
        )
        self._init_densities()
    ...
    def __repr__(self) -> str:
        return (
            f"[{self.offset_blocks_start}-{self.offset_blocks_end}; tl={self.tag_level}; nw={self.num_words};"
