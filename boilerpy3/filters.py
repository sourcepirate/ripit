...
class BoilerpipeFilter:
    ...
    def subtract_blocks(
        self, block_arr: List[TextBlock], blocks_to_remove: List[TextBlock]
    ) -> List[TextBlock]:
        """
...
class MinClauseWordsFilter(BoilerpipeFilter):
    ...
    def __init__(
        self,
        min_words: int = 5,
        accept_clauses_without_delimiter: bool = False,
    ) -> None:
...
class SurroundingToContentFilter(BoilerpipeFilter):
    ...
    def process(self, doc: TextDocument) -> bool:
        ...
        while i < n - 1:
            ...
            next_block = tbs[i + 1]
            if (
                not cur_block.is_content
                and prev_block.is_content
                and next_block.is_content
                and self.cond(cur_block)
            ):
...
class ContentFusion(BoilerpipeFilter):
    def process(self, doc: TextDocument) -> bool:
        ...
        while changed_on_pass:
            ...
            for block in text_blocks[1:]:
                if (
                    prev_block.is_content
                    and block.link_density < 0.56
...
class BlockProximityFusion(BoilerpipeFilter):
    ...
    def __init__(
        self,
        max_blocks_distance: int = 1,
        content_only: bool = False,
        same_tag_level_only: bool = False,
    ) -> None:
        """
...
class KeepLargestBlockFilter(BoilerpipeFilter):
    ...
    def process(self, doc: TextDocument) -> bool:
        ...
        try:
            largest_block = max(
                (tb for tb in text_blocks if tb.is_content),
                key=lambda tb: tb.num_words,
            )
        except ValueError:
...
class ArticleMetadataFilter(BoilerpipeFilter):
    ...
    PATTERNS_SHORT = [
        re.compile(
            r"^[0-9 ,./]*\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|January|February|March|April|June|"
...
class IgnoreBlocksAfterContentFilter(HeuristicFilterBase):
    ...
    def process(self, doc: TextDocument) -> bool:
        ...
        for block in doc.text_blocks:
            ...
            if (
                block.has_label(DefaultLabels.INDICATES_END_OF_TEXT)
                and num_words >= self.min_num_words
            ):
...
class CanolaFilter(BoilerpipeFilter):
    ...
    def classify(
        ...
        cond3 = (
            next_block.num_words > 6
            and next_block.link_density == 0
            and prev_block.link_density == 0
            and (
                curr_block.num_words > 6
                or prev_block.num_words > 7
                or next_block.num_words > 19
            )
        )
